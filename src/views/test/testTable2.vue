<template>
    <LayoutMain>
        ...
        <template slot="topRight">
            <el-input class="el_input_search" v-model="searchForm.yhmc" size="mini" placeholder="请输入用户姓名"/>
            <el-input class="el_input_search" v-model="searchForm.dwmc" size="mini" placeholder="请输入单位名称"/>
        ...
        <template>
            ...
            >
             ...
             <div>
                <TestTableAvueAddForm  class="form_test" :avueAddShow.sync="avueAddShow" :reFresh="refreshChange"></TestTableAvueAddForm>
                <!--只有在一开始不需要渲染dom元素的组件父标签上加v-if v-if消耗浏览器加载速度  v-if控制dom元素是否加载 如果不加v-if那么一渲染浏览器 标签内的dom元素就开始渲染 -->
                <TestTableAvueForm v-if="avueShow" class="form_test" :formData="rowdata" :avueShow.sync="avueShow"  :reFresh="refreshChange"></TestTableAvueForm>
                <SelTysqAnyRy v-if="tysqShow" class="form_test" :tysqShow.sync="tysqShow"   @sendData="test"></SelTysqAnyRy>
            </div>
            <div>
...
<script>
  import testForm from "./formPlugn/TestForm";
  import iconShow from "./formPlugn/IconShow";
  import TestTableAvueForm from "./formPlugn/TestTableAvueForm";
  import TestTableAvueAddForm from "./formPlugn/TestTableAvueAddForm";
  import {deepClone,isEmpty,getArrayStrValue,downLoadFile} from "@/util/util";
  ...
  export default { 
    ...
    mounted() {
      //采用链式监听才会触发不然 内置多个onresize方法会有问题
      // window.addEventListener('resize',() => {
      //       this.screenHeight = window.innerHeight;
      //       this.option = Object.assign({}, this.option, {height: document.body.clientHeight-215});
      // }, false);
    },
    watch:{
        // screenHeight (val) {
        ...
        //       this.option = Object.assign({}, this.option, {height: val-215});
        //       this.timer = true;
        //       let that = this;
        //       setTimeout(function () {
    ...
    methods: {
        ...
        handleCurrentRowChange(val,oldval){//行选中
            this.tempData=deepClone(val);
        },
        remove(){//清除选中状态
            this.$refs.singleTable.setCurrentRow();
        },
        handleRowSave(row,done,loading){//新增保存
            var that=this;
            this.axios.apiPost({
              url:'/test/saveData',
              data:row,
        ...
        sortChange(column){//自定义排序
            this.page.orderColumn=column.prop;
            this.page.order=column.order==null?"":column.order=="ascending"?"asc":"desc";
            this.refreshData();
        },
        download(){
