...
//HTTPresponse拦截
_axios2.default.interceptors.response.use(res=> {
  ...
  if (response) {
    ...
    if(status===401){//hhid验证未通过
      ...
      this.$message({
        type: "error",
        message: `404 not found: ${response.data.path}`,
        duration: 0,
      });
    }else if(status===502){
      this.$message({
        type: "error",
        message: `502 bad gateway: 服务器重启中,请10分钟以后再试...`,
        duration: 0,
      });
    }else if(status===504){
      this.$message({
        type: "error",
        message: `504 Gateway Time-out: 请求超时`,
        duration: 0,
      });
    }else{
      this.$message({
        type: "error",
        message: `后台服务异常: ${response.data.message||JSON.stringify(response.data)}`,
        duration: 0,
      });
    }
