'use strict';

exports.__esModule = true;
exports.getRoleTree = exports.getRole = exports.update = exports.add = exports.remove = exports.grant = exports.grantTree = exports.getList = undefined;

var _extends = Object.assign || function (target) { for (var i = 1; i < arguments.length; i++) { var source = arguments[i]; for (var key in source) { if (Object.prototype.hasOwnProperty.call(source, key)) { target[key] = source[key]; } } } return target; };

var _axios = require('npm-pansoft-base/lib/router/axios');

var _axios2 = _interopRequireDefault(_axios);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var getList = exports.getList = function getList(current, size, params) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/list',
    method: 'get',
    params: _extends({}, params, {
      current: current,
      size: size
    })
  });
};
var grantTree = exports.grantTree = function grantTree() {
  return (0, _axios2.default)({
    url: '/api/blade-system/menu/grant-tree',
    method: 'get'
  });
};

var grant = exports.grant = function grant(roleIds, menuIds) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/grant',
    method: 'post',
    params: {
      roleIds: roleIds,
      menuIds: menuIds
    }
  });
};

var remove = exports.remove = function remove(ids) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/remove',
    method: 'post',
    params: {
      ids: ids
    }
  });
};

var add = exports.add = function add(row) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/submit',
    method: 'post',
    data: row
  });
};

var update = exports.update = function update(row) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/submit',
    method: 'post',
    data: row
  });
};

var getRole = exports.getRole = function getRole(roleIds) {
  return (0, _axios2.default)({
    url: '/api/blade-system/menu/role-tree-keys',
    method: 'get',
    params: {
      roleIds: roleIds
    }
  });
};

var getRoleTree = exports.getRoleTree = function getRoleTree(tenantId) {
  return (0, _axios2.default)({
    url: '/api/blade-system/role/tree',
    method: 'get',
    params: {
      tenantId: tenantId
    }
  });
};